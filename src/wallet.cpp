...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                // if sub-cent change is required, the fee must be raised to at least transaction fee
                // or until nChange becomes zero
                // NOTE: this depends on the exact behaviour of GetMinFee
                if (nFeeRet < GetTxFee() && nChange > 0 && nChange < CENT)
                {
                    int64_t nMoveToFee = min(nChange, GetTxFee() - nFeeRet);
                    nChange -= nMoveToFee;
                    nFeeRet += nMoveToFee;
                }
